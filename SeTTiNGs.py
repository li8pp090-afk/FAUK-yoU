...
def build_filename(info: dict, actual_path: Path) -> str:
    publisher_raw = info.get("channel") or info.get("uploader") or info.get("creator") or ""
    publisher = clean_component(publisher_raw)

    title_raw = info.get("title") or ""
    title = clean_component(title_raw)
...
def is_ignored_url(url: str) -> bool:
    try:
        ...
        return host in IGNORED_HOSTS or host.endswith(".telegram.org") or host.endswith(".youtube.com")
    except Exception:
        return False
